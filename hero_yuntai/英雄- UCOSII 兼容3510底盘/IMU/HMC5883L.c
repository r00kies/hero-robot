...
#include "mpu6050.h"
float x_scale,y_scale,z_scale,x_max,y_max,z_max,mxyz[3];
...
void HMC58X3_writeReg(unsigned char reg, unsigned char val) {
  IIC_WriteData(HMC58X3_ADDR,reg,val);
}
...
//   IICreadBytes(HMC58X3_ADDR,HMC58X3_R_XM,6,vbuff);
    IIC_ReadData( HMC58X3_ADDR,HMC58X3_R_XM,vbuff,6);
...
void HMC58X3_getID(char id[3]) 
{
 ...
 IIC_ReadData( HMC58X3_ADDR,HMC58X3_R_IDA,(u8 *)id,3);    
