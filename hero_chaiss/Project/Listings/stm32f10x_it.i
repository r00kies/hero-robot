...
#line 32 "..\\User\\stm32f10x_it.c"

uint16_t SystemCounter = 0;
uint8_t Moto_pid_1000Hz = 0;
uint8_t Attitude_500Hz = 0;
uint8_t Motor_100Hz = 0;
uint8_t Com_10Hz = 0;
uint8_t Set_2Hz = 0;
...
void SysTick_Handler(void)
{
  ...
  {
    SystemCounter++;
    if(SystemCounter > 500)
    {SystemCounter = 1;}
		
    if((SystemCounter % 1) == 0)	   
    {Moto_pid_1000Hz = 1;}
		
    if((SystemCounter % 2) == 0)	   
    {Attitude_500Hz = 1;}

    if((SystemCounter % 10) == 0)
    {Motor_100Hz = 1;}

	  if((SystemCounter % 100) == 0)
  	{Com_10Hz = 1;}
		if((SystemCounter % 500) == 0)
	  {Set_2Hz = 1;} 
