...
void TIM4_Init()
{
  ...
  RCC_APB2PeriphClockCmd(((uint32_t)0x00000001), ENABLE);
	...
	TIM_InitStructure.TIM_CounterMode=((uint16_t)0x0000);
	TIM_InitStructure.TIM_ClockDivision=((uint16_t)0x0200);
	TIM_InitStructure.TIM_RepetitionCounter=0;
	TIM_TimeBaseInit(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)),&TIM_InitStructure);
...
void TIM4_IRQHandler(void)
{
	...
	{
		static float out_pwm,AD_out_pwm;
        TIM_ClearITPendingBit(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)),((uint16_t)0x0001));
        Moto_Drive.power=AD_Power();
        
        Moto_Drive.Measured_Speed=Encoder_Get_CNT()*Moto_Drive.Encoder_Dir;
        if(Moto_Drive.power>Moto_Drive.limit_power)
        AD_out_pwm=PID_Calculate(&PID_Moto_Drive,Moto_Drive.power,Moto_Drive.limit_power);
        else
        {
        AD_out_pwm=0;
        PID_Moto_Drive.SumErr=0;
        }
        
	    out_pwm=PID_Calculate(&PID_Moto_Drive,Moto_Drive.Measured_Speed,Moto_Drive.Speed_Value);
        if(out_pwm>0)
         out_pwm+=AD_out_pwm;
        else
         out_pwm-=AD_out_pwm;             
        MotorSpeedOut(out_pwm*Moto_Drive.Moto_Dir);
  } 
}
